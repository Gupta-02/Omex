...
import { FaChevronDown, FaChevronUp,FaGithub , FaUsers, FaHeart, FaBook, FaTerminal , FaEye, FaCheckCircle, FaCheck, FaCopy } from "react-icons/fa";
import { IoAlertCircle,IoGitCommit,IoGitPullRequest   } from "react-icons/io5";
import { FiFileText,FiMessageSquare,FiZap,FiExternalLink } from "react-icons/fi";
...
const ContributorGuide = () => {
  ...
  const steps = [
    {
      icon: <IoIosGitBranch className="w-6 h-6" />,
    ...
    {
      icon: <IoGitCommit className="w-6 h-6" />,
      title: 'Make Changes',
    ...
    {
      icon: <IoGitPullRequest  className="w-6 h-6" />,
  ...
  const prSteps = [
    {
      icon: <IoGitPullRequest  className="w-7 h-7" />,
  ...
  const faqs = [
    {
      ...
      category: 'basics',
      icon: <IoIosGitBranch className="w-5 h-5" />
    },
    {
      ...
      category: 'basics',
      icon: <IoGitPullRequest  className="w-5 h-5" />
    },
    {
    ...
    },
    {
      ...
      category: 'workflow',
      icon: <IoGitCommit className="w-5 h-5" />
    },
    {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-800'
      }`}>
      {/* Hero Section */}
      <div className="bg-blue-500 text-white py-16">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <div className="flex items-center justify-center mb-6">
            <FaHeart className="w-8 h-8 mr-3 text-red-400" />
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <section className="mb-16">
          ...
          <div className={`rounded-2xl p-4 md:p-8 shadow-xl border ${isDark
            ...
            <div className="space-y-6 md:space-y-8">
              {prSteps.map((step, index) => (
                <div key={index} className="flex flex-col md:flex-row md:items-start space-y-4 md:space-y-0 md:space-x-6 group">
                  ...
                  <div className={`flex-1 rounded-xl p-4 md:p-6 shadow-lg hover:shadow-xl transition-all duration-300 backdrop-blur-sm border ${isDark
                    ...
                    <h3 className={`text-lg md:text-xl font-bold mb-3 ${isDark ? 'text-white' : 'text-gray-800'
                      }`}>{step.title}</h3>
                    <p className={`mb-4 text-sm md:text-base ${isDark ? 'text-gray-400' : 'text-gray-600'
                      }`}>{step.description}</p>
                    <div className={`rounded-lg p-3 border-l-4 border-blue-500 ${isDark ? 'bg-gray-900' : 'bg-gray-100'
                      }`}>
              ...
              <div key={categoryKey} className="mb-8">
                ...
                <div className="space-y-3">
                  {categoryFaqs.map((faq, index) => {
                    ...
                    return (
                      <div key={faqIndex} className={`rounded-xl shadow-lg overflow-hidden border hover:shadow-xl transition-all duration-300 ${isDark
                          ? 'bg-gray-800 border-gray-700'
                          : 'bg-white border-gray-300'
                        }`}>
                        <button
                          onClick={() => setExpandedFAQ(expandedFAQ === faqIndex ? null : faqIndex)}
                          className={`w-full px-6 py-4 text-left flex items-center justify-between transition-all duration-300 group ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-50'
                            }`}
                        >
                          <div className="flex items-center space-x-4">
                            ...
                            <h3 className={`text-lg font-semibold group-hover:text-blue-500 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-800'
                              }`}>
                              {faq.question}
                            </h3>
                          </div>
                          <div className="text-blue-500 group-hover:scale-110 transition-transform duration-300">
                            {expandedFAQ === faqIndex ? <FaChevronUp className="w-5 h-5" /> : <FaChevronDown className="w-5 h-5" />}
                          </div>
                        </button>
                        {expandedFAQ === faqIndex && (
                          <div className={`px-6 pb-6 border-t ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'
                            }`}>
                            <div className={`rounded-lg p-4 mt-4 backdrop-blur-sm border ${isDark
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {resources.map((resource, index) => (
              <a
                ...
                className={`rounded-xl shadow-lg p-6 hover:shadow-xl transition-all duration-500 transform hover:-translate-y-1 hover:scale-105 group border ${isDark
                    ? 'bg-gray-800 border-gray-700'
                    : 'bg-white border-gray-300'
                  }`}
              >
                <div className="flex items-center mb-4">
                  ...
                  <FiExternalLink className={`w-4 h-4 group-hover:text-blue-500 transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-600'}`} />
                </div>
                <h3 className={`text-lg font-bold mb-2 group-hover:text-blue-500 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-800'
                  }`}>{resource.title}</h3>
        ...
        <div className="bg-blue-500 rounded-xl p-8 text-center text-white shadow-xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              ...
              className={`px-8 py-3 rounded-lg font-semibold transition-all duration-300 hover:scale-105 hover:shadow-xl flex items-center justify-center transform hover:-translate-y-0.5 ${isDark
